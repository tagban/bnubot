...
public class HTTPServer extends Thread {
	...
	public void run() {
		try {
			...
			Out.error("HTTP", "Could not bind port " + Constants.HTTPPort
					+ ". HTTP server is disabled.");
			return;
		} catch (IOException e) {
			Out
					.error("HTTP", "Could not create socket (Port: "
							+ Constants.HTTPPort
		...
		listening = true;
		Out.println("HTTP", "Server socket opened on port "
				+ Constants.HTTPPort);
		while (listening) {
			try {
	...
	public static String buildHeader(int status, String mimeType,
			long contentLong) {
