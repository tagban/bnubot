...
/**
 * This class is the actual BNLS server. It creates the server socket, and
 * accepts connections on it, spawning a new thread for every connection.
 */
public class BNLSServer extends Thread {
	...
	public void run() {
		try {
			...
			Out.error("JBLS", "Could not bind port " + Constants.BNLSPort
					+ ". JBLS server is disabled.");
			return;
		} catch (IOException e) {
			Out.error("JBLS", "Could not create socket (Port: "
					+ Constants.BNLSPort + ")  - Shutting down JBLS. \n\r"
					+ e.toString());
			Controller.disableJBLSServer();
			return;
		}
		listening = true;
		Out.println("JBLS", "Server socket opened on port "
				+ Constants.BNLSPort);

		while (listening) {
			try {
				Out.info("JBLS", "Listening for new Connection...");
				Socket inSocket = server.accept();// block until a connection
													// is made

				BNLSConnectionThread bConnection = new BNLSConnectionThread(
						inSocket); // Create the new thread
				...
				Out
						.error("JBLS", "Could not Accept Connection "
								+ e.toString());
				listening = false;
				break;
			}// end try-catch
