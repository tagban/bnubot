...
public class BNFTP extends Thread {
	...
	public static void main(String[] args) {
		BNFTP ftp = new BNFTP("uswest.battle.net", 6112,
				"D2DV_IX86_111A_111B.mpq");
		ftp.start();
	}
	...
	public BNFTP(String bnServer, int bnPort, String fileName, BotNet callback,
			int userid) {
		this.server = bnServer;
		this.port = bnPort;
		this.file = fileName;
	...
	public String BNFTP(String bnServer, int bnPort, String fileName,
		...
		try {
			...
			if (callback != null)
				callback.whisper(userid, "BNFTP: Sending request for "
						+ fileName);
			...
			try {
				...
				if (callback != null)
					callback.whisper(userid, "BNFTP: " + fileName
							+ " Recived File Size " + fileSize);
				...
				while (bytesRead < fileSize) {
					...
					if (i == -1)
						throw new IOException(
								"Failed to recive all file data. Last offset: "
										+ bytesRead);
					file.write((byte) i);
					bytesRead++;
				}
				file.flush();
				file.close();
				new File(Constants.DownloadPath + fileName)
						.setLastModified(fileTimeToMillis(filetime));
				Out.println("BNFTP", "Recived file: " + fileName + " Size: "
						+ fileSize);
				if (callback != null)
					callback.whisper(userid, "BNFTP: Recived file: " + fileName
							+ " Size: " + fileSize);
			...
			Out.println("BNFTP", "Could not find host: " + bnServer + ":"
					+ bnPort);
			if (callback != null)
				callback.whisper(userid, "BNFTP: Could not find host: "
						+ bnServer + ":" + bnPort);
