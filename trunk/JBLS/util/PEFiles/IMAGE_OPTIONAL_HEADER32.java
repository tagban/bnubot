...
public class IMAGE_OPTIONAL_HEADER32 extends HEADER {
	/***************************************************************************
	 * typedef struct _IMAGE_OPTIONAL_HEADER { * // Standard fields. * WORD
	 * Magic; : 0x00* BYTE MajorLinkerVersion; : 0x02* BYTE MinorLinkerVersion; :
	 * 0x03* DWORD SizeOfCode; : 0x04* DWORD SizeOfInitializedData; : 0x08*
	 * DWORD SizeOfUninitializedData; : 0x0c* DWORD AddressOfEntryPoint; : 0x10*
	 * DWORD BaseOfCode; : 0x14* DWORD BaseOfData; : 0x18* // NT additional
	 * fields. * DWORD ImageBase; : 0x1c* DWORD SectionAlignment; : 0x20* DWORD
	 * FileAlignment; : 0x24* WORD MajorOperatingSystemVersion; : 0x28* WORD
	 * MinorOperatingSystemVersion; : 0x2a* WORD MajorImageVersion; : 0x2c* WORD
	 * MinorImageVersion; : 0x2e* WORD MajorSubsystemVersion; : 0x30* WORD
	 * MinorSubsystemVersion; : 0x32* DWORD Win32VersionValue; : 0x34* DWORD
	 * SizeOfImage; : 0x38* DWORD SizeOfHeaders; : 0x3c* DWORD CheckSum; : 0x40*
	 * WORD Subsystem; : 0x44* WORD DllCharacteristics; : 0x46* DWORD
	 * SizeOfStackReserve; : 0x48* DWORD SizeOfStackCommit; : 0x4c* DWORD
	 * SizeOfHeapReserve; : 0x50* DWORD SizeOfHeapCommit; : 0x54* DWORD
	 * LoaderFlags; : 0x58* DWORD NumberOfRvaAndSizes; : 0x5c*
	...
	public IMAGE_OPTIONAL_HEADER32(byte[] data) {
		create(data, 0);
	}

	public IMAGE_OPTIONAL_HEADER32(char[] data) {
		create(toByteArray(data), 0);
	}

	public IMAGE_OPTIONAL_HEADER32(byte[] data, int offset) {
		create(data, offset);
	}

	public IMAGE_OPTIONAL_HEADER32(char[] data, int offset) {
		create(toByteArray(data), offset);
	}
	...
	public void create(byte[] data, int offset) {
		...
		for (int x = 0; x < IMAGE_NUMBEROF_DIRECTORY_ENTRIES; x++)
			DataDirectory[x] = new IMAGE_DATA_DIRECTORY(data, offset + 0x60
					+ (x * 0x08));
	}
	...
	public void print(PrintStream out, int indent) {
		...
		out.println(i + "  Linker Version:           " + MajorLinkerVersion
				+ "." + MinorLinkerVersion);
		out.println(i + "  Code Size:                0x" + hex(SizeOfCode));
		out.println(i + "  Initialized Data Size:    0x"
				+ hex(SizeOfInitializedData));
		out.println(i + "  Uninitialized Data Size:  0x"
				+ hex(SizeOfUninitializedData));
		out.println(i + "  Entry Point:              0x"
				+ hex(AddressOfEntryPoint));
		...
		out.println(i + "  Section Alignment:        0x"
				+ hex(SectionAlignment));
		out.println(i + "  File Alignment:           0x" + hex(FileAlignment));
		out.println(i + "  Operating System Version: "
				+ MajorOperatingSystemVersion + "."
				+ MinorOperatingSystemVersion);
		out.println(i + "  Image Version:            " + MajorImageVersion
				+ "." + MinorImageVersion);
		out.println(i + "  Subsystem Version:        " + MajorSubsystemVersion
		...
		out.println(i + "  DLL Characteristics:      0x"
				+ hex(DllCharacteristics));
		out.println(i + "  Stack Reserve Size:       0x"
				+ hex(SizeOfStackReserve));
		out.println(i + "  Stack Commit Size:        0x"
				+ hex(SizeOfStackCommit));
		out.println(i + "  Heap Reserve Size:        0x"
				+ hex(SizeOfHeapReserve));
		out.println(i + "  Heap Commit Size:         0x"
				+ hex(SizeOfHeapCommit));
		out.println(i + "  Loader Flags:             0x" + hex(LoaderFlags));
		out.println(i + "  Number of Rva and Sizes:  " + NumberOfRvaAndSizes);
		for (IMAGE_DATA_DIRECTORY element : DataDirectory)
