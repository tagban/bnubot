...
public class IMAGE_IMPORT_DIRECTORY extends HEADER {
	/***************************************************************************
	 * typedef struct _IMAGE_IMPORT_DIRECTORY { * DWORD
	 * AddressOfImportLookupTable; : 0x00* DWORD TimeDateStamp; : 0x04* DWORD
	 * ForwarderChain; : 0x08* DWORD AddressOfNames; : 0x0c* DWORD
	 * AddressOfImportTable; : 0x10* } *
	 **************************************************************************/
	...
	public IMAGE_IMPORT_DIRECTORY(byte[] data) {
		create(data, 0);
	}

	public IMAGE_IMPORT_DIRECTORY(char[] data) {
		create(toByteArray(data), 0);
	}

	public IMAGE_IMPORT_DIRECTORY(byte[] data, int offset) {
		create(data, offset);
	}

	public IMAGE_IMPORT_DIRECTORY(char[] data, int offset) {
	...
	public void print(PrintStream out, int indent) {
		...
		out.println(i + "  Lookup Table Address: 0x"
				+ hex(AddressOfImportLookupTable));
		...
		out.println(i + "  Import Table Address: 0x"
				+ hex(AddressOfImportTable));
