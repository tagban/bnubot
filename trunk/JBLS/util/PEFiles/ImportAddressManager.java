...
public class ImportAddressManager {
	...
	public static int getAddress(String function) {
		...
		if (ret != null)
			return ret.intValue();
		if (forward != null)
			return getAddress(forward.toLowerCase());
	...
	public static void loadLibrary(String file, String prepend) {
		...
		IMAGE_EXPORT_DIRECTORY dir = new IMAGE_EXPORT_DIRECTORY(
				pe.getSectionsData(),
				pe.ntheader.OptionalHeader.DataDirectory[pe.EXPORT_TABLE].VirtualAddress);
		...
		int low = pe.ntheader.OptionalHeader.DataDirectory[pe.EXPORT_TABLE].VirtualAddress;
		int high = low
				+ pe.ntheader.OptionalHeader.DataDirectory[pe.EXPORT_TABLE].Size;

		for (int x = 0; x < dir.NumberOfNames; x++) {
			if ((addrs[x] > low) && (addrs[x] < high)) {
				forwards.put((prepend + "." + names[x]).toLowerCase(), pe
						.getStringSection(addrs[x]));
				...
				addresses.put((prepend + "." + names[x]).toLowerCase(),
						new Integer(addrs[x]
								+ pe.ntheader.OptionalHeader.ImageBase));
				// System.out.println((prepend+"."+names[x]).toLowerCase() +
				// "->" +
