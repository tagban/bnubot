...
/*******************************************************************************
 * BotID: RivalBot * HubPW: b8f9b319f223ddcc38 * Databse: JBLS * Password:
 * 0a9115d19f4cf125 * Database: MiniCouncil * Password:fbf372f087c6dc1626d This
 * will allow an admin * interface to JBLS through * botnet. *
 ******************************************************************************/
public class BotNet extends Thread {
	...
	public BotNet(String account, String password) {
	...
	public void run() {
		try {
			...
			while (true) {
				...
				int id = in.read();
				short plength = (short) (((in.read() << 0) & 0x000000FF) | ((in
						.read() << 8) & 0x0000FF00));
				if (id == -1) {
	...
	private void parse(Buffer data, OutputStream out, int ID) {
		...
		case PACKET_USERINFO:
			...
			String database = data.removeNTString();
		...
		case PACKET_COMMAND:
			...
			short goodLen = data.removeWord();
			short extra = data.removeWord();
			if (error == 1)
				Out.debug("BotNet", "Command error: Unknown packet ID: " + id);
		...
		case PACKET_BOTNETVERSION:
			if (Constants.debugInfo) {
				...
				case 0x04:
					Out.debug("BotNet", "Server Version: 4");
				}
			}
			break;
		...
		case PACKET_BOTNETCHAT:
			...
			int action = data.removeDWord();
			id = data.removeDWord();
			String message = data.removeNTString();
			if (command == 2)
				parseCommand(id, message, out);
			break;
		...
		case PACKET_ACCOUNT:
			...
			case 0x00:
				if (success) {
					Out.debug("BotNet", "Logged in to account "
							+ Constants.BotNetUsername);
					buff = new OutPacketBuffer(PACKET_USERINFO);
					send(buff, out);
				} else {
					Out.debug("BotNet",
							"Failed to login, attempting to create account "
									+ username);
					buff = new OutPacketBuffer(PACKET_ACCOUNT);
					buff.addDWord(2);
					buff.addNTString(Constants.BotNetUsername);
			...
			case 0x02:
				if (success) {
					...
					Out
							.debug("BotNet",
									"Failed to create account. Terminating connection.");
					return;
				}
				break;
			}
		...
		default:
			Out.info("BotNet", "Unknown Packet ID: " + Integer.toHexString(ID));
		}
	}
	...
	private void parseCommand(int ID, String message, OutputStream out) {
		if (names.get(ID) == null)
			return;
		int access = Integer.parseInt(Ini.ReadIni("./admins.ini", "Admins",
				names.get(ID), "0"), 10);
		if (access == 0)
			return;
		...
		if (args[0].equalsIgnoreCase("respond")) {
			response
					.append("Fuck you man! I'm here, I responded, now leave me alone!\r");
			...
			BNLSProtocol.IpAuth.setIpStatus(args[1],
					BNLSProtocol.IpAuth.IPBANNING);
			response.append("IP ").append(args[1]).append(" is now Banned.\r");
		} else if (args[0].equalsIgnoreCase("IPAuth")) {
			response.append(BNLSProtocol.IpAuth.getIpString(args[1])).append(
					"\r");
		} else if (args[0].equalsIgnoreCase("set")) {
			Ini.WriteIni("./settings.ini", args[1], args[2], message
					.substring(args[1].length() + args[2].length() + 6));
			...
			if (args[1].equalsIgnoreCase("all")) {
				if (Controller.hServer != null)
					Controller.hServer.destroyAllConnections();
				if (Controller.jServer != null)
					Controller.jServer.destroyAllConnections();
				response
						.append("All HTTP and JBLS connection threads terminated.\r");
				...
				response
						.append("Unknown command, Propper usage: kill [All|HTP|JBLS]\r");
		} else if (args[0].equalsIgnoreCase("stop")) {
			if (args[1].equalsIgnoreCase("all")) {
				// if(Controler.hServer != null)
				// Controler.hServer.destroyAllConnections();
				Controller.stopJBLS();
				...
				response
						.append("Unknown command, Propper usage: stop [All|HTP|JBLS]\r");
			...
			if (args[1].equalsIgnoreCase("all")) {
				...
				response
						.append("Unknown command, Propper usage: start [All|HTP|JBLS]\r");
			...
			if (args[1].equalsIgnoreCase("all")) {
				...
				response
						.append("Unknown command, Propper usage: restart [All|HTP|JBLS]\r");
		} else if (args[0].equalsIgnoreCase("shutdown")) {
			whisper(ID,
					"Server shutting down, Goodbye crul world!!! I will miss thee!");
			Controller.shutdown();
		} else if (args[0].equalsIgnoreCase("help")) {
			response.append("JBLS Admin Commands:\r");
			response
					.append("  BNFTP [Server] [Files ...] - The JBLS server will attempt to download these files useing Battle.net's FTP protocol.\r");
			response.append("  Help - This message.\r");
			response
					.append("  IPAuth [IP] - Displays the statistice for the specified IP\r");
			response
					.append("  IPBan [IP] - Adds the specified IP to the IPBan list.\r");
			response
					.append("  IPs - List all IP address that have special atributes on the server.\r");
			response
					.append("  Kill [All|HTTP|JBLS] - Destroys all client threads associated with the specified server.\r");
			response.append("  Respond - Test command, Don't use.\r");
			response
					.append("  Restart [All|HTTP|JBLS] - Restarts the server, Kills all connections and opens up for new ones.\r");
			response
					.append("  Set [Header] [Property] [Value] - Raw Settings.ini access, Writes the data to Settings.ini.\r");
			response
					.append("  SetIp [IP] [Status] - Manually sets the status of the IP to the number specified.\r");
			response
					.append("  Shutdown - Destroys all connection threads, and exits the daemon.\r");
			response
					.append("  Start [All|HTTP|JBLS] - Enable's the noted server.\r");
			response
					.append("  Stop [All|HTTP|JBLS] - Disable's the noted server.\r");
		} else {
	...
	private class OutPacketBuffer extends Buffer {
		...
		public byte[] getBuffer() {
			...
			System.arraycopy(super.getBuffer(), 0, ret, 4, size());// Copy
																	// Buffer
																	// Into a
																	// New Array
