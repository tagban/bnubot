...
public class seed_finder {
	...
	public static byte[] lockdown_values = new byte[] { (byte) 0x81,
			(byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF,
			(byte) 0x35, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF,
			(byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF,
			(byte) 0xFF, (byte) 0x6a, (byte) 0x08 };
	...
	public static int[] find_seeds(String file, byte[] values, int[] offsets) {
		...
		for (int i = 0; i < data.length - values.length; i++) {
			...
			for (int j = 0; (j < values.length) && found; j++)
				if ((values[j] != (byte) 0xFF) && (data[i + j] != values[j]))
					found = false;
