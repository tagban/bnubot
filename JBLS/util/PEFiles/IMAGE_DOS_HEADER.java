...
public class IMAGE_DOS_HEADER extends HEADER {
	/***************************************************************************
	 * typedef struct _IMAGE_DOS_HEADER { // DOS .EXE header * USHORT e_magic; //
	 * Magic number : 0x00* USHORT e_cblp; // Bytes on last page of file : 0x02*
	 * USHORT e_cp; // Pages in file : 0x04* USHORT e_crlc; // Relocations :
	 * 0x06* USHORT e_cparhdr; // Size of header in paragraphs : 0x08* USHORT
	 * e_minalloc; // Minimum extra paragraphs needed : 0x0a* USHORT e_maxalloc; //
	 * Maximum extra paragraphs needed : 0x0c* USHORT e_ss; // Initial
	 * (relative) SS value : 0x0e* USHORT e_sp; // Initial SP value : 0x10*
	 * USHORT e_csum; // Checksum : 0x12* USHORT e_ip; // Initial IP value :
	 * 0x14* USHORT e_cs; // Initial (relative) CS value : 0x16* USHORT
	 * e_lfarlc; // File address of relocation table : 0x18* USHORT e_ovno; //
	 * Overlay number : 0x1a* USHORT e_res[4]; // Reserved words : 0x1c* USHORT
	 * e_oemid; // OEM identifier (for e_oeminfo) : 0x24* USHORT e_oeminfo; //
	 * OEM information; e_oemid specific : 0x26* USHORT e_res2[10]; // Reserved
	 * words : 0x28* LONG e_lfanew; // File address of new exe header : 0x3c* } *
	 **************************************************************************/
	...
	public IMAGE_DOS_HEADER(char[] data) {
		create(toByteArray(data), 0);
	}

	public IMAGE_DOS_HEADER(byte[] data, int offset) {
		create(data, offset);
	}

	public IMAGE_DOS_HEADER(char[] data, int offset) {
