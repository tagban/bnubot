...
public class PEFile {
	...
	private void fillIAT(IMAGE_DATA_DIRECTORY dir) {
		...
		IMAGE_IMPORT_DIRECTORY iid = new IMAGE_IMPORT_DIRECTORY(sections,
				dir.VirtualAddress);
		int x = 1;
		while (iid.AddressOfName != 0) {
			...
			while (getIntSection(iid.AddressOfImportLookupTable + (4 * y)) != 0) {
				...
				if ((fp > sections.length) || (fp < 0))
					...
					address = ImportAddressManager.getAddress(library + "."
							+ function);
				}

				// System.out.println(Integer.toHexString(address) + ": " +
				// function + ": " +
				// Integer.toHexString(getIntSection(iid.AddressOfImportTable+(4*y))));
				if (address != 0)
					putSection(address, iid.AddressOfImportTable + (4 * y));
				y++;
			}

			iid = new IMAGE_IMPORT_DIRECTORY(sections, dir.VirtualAddress
					+ (x * IMAGE_SIZEOF_IMPORT_HEADER));
			x++;
		}
		System.out
				.println(getStringSection(getIntSection(iid.AddressOfImportTable)));
	}

	private void loadSection(IMAGE_SECTION_HEADER section, char[] buffer) {
		char[] data = getData(
				section.PointerToRawData,
				(section.SizeOfRawData > section.VirtualSize ? section.VirtualSize
	...
	public PEFile(String fn) {
		...
		int sectionStart = ntheader.FileHeader.SizeOfOptionalHeader
				+ dosheader.e_lfanew + SIZE_OF_NT_SIGNATURE
		...
		for (int x = 0; x < ntheader.FileHeader.NumberOfSections; x++) {
			sectionHeaders[x] = new IMAGE_SECTION_HEADER(data, sectionStart
					+ (IMAGE_SIZEOF_SECTION_HEADER * x));
	...
	public void putSection(int data, int offset) {
		...
		// System.out.println("Changing: 0x" + Integer.toHexString(offset) + "
		// to 0x" + Integer.toHexString(data));
	}

	public char[] getSectionsData() {
		return sections;
	}
	...
	public char[] getDataSection(int start, int length) {
		char[] buff = new char[length];
		for (int x = 0; x < length; x++)
			buff[x] = sections[start + x];
		return buff;
	}

	public String getStringSection(int o) {
		return getString(sections, o);
	}

	public String getString(byte[] d, int o) {
		int start = o;
		while ((d[o] != 0) && (o < d.length))
			o++;
	...
	public int getIntSection(int o) {
		return getInt(sections, o);
	}

	public int getInt(int o) {
		return getInt(data, o);
	}
	...
	public short getShortSection(int o) {
		return getShort(sections, o);
	}

	public short getShort(int o) {
		return getShort(data, o);
	}
