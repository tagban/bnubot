...
public class IMAGE_NT_HEADERS extends HEADER {
	/***************************************************************************
	 * typedef struct _IMAGE_NT_HEADERS { * DWORD Signature; : 0x00*
	 * IMAGE_FILE_HEADER FileHeader; : 0x04* IMAGE_OPTIONAL_HEADER32
	 * OptionalHeader; : 0x18* } *
	 **************************************************************************/
	...
	public IMAGE_NT_HEADERS(byte[] data) {
		create(data, 0);
	}

	public IMAGE_NT_HEADERS(char[] data) {
		create(toByteArray(data), 0);
	}

	public IMAGE_NT_HEADERS(byte[] data, int offset) {
		create(data, offset);
	}

	public IMAGE_NT_HEADERS(char[] data, int offset) {
