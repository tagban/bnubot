...
public class IMAGE_FILE_HEADER extends HEADER {
	/***************************************************************************
	 * typedef struct _IMAGE_FILE_HEADER { * USHORT Machine; : 0x00* USHORT
	 * NumberOfSections; : 0x02* ULONG TimeDateStamp; : 0x04* ULONG
	 * PointerToSymbolTable; : 0x08* ULONG NumberOfSymbols; : 0x0c* USHORT
	 * SizeOfOptionalHeader; : 0x10* USHORT Characteristics; : 0x12* } *
	 **************************************************************************/
	...
	public IMAGE_FILE_HEADER(byte[] data) {
		create(data, 0);
	}

	public IMAGE_FILE_HEADER(char[] data) {
		create(toByteArray(data), 0);
	}

	public IMAGE_FILE_HEADER(byte[] data, int offset) {
		create(data, offset);
	}

	public IMAGE_FILE_HEADER(char[] data, int offset) {
	...
	public void print(PrintStream out, int indent) {
		...
		out.println(i + "  Symbol Table Address: 0x"
				+ hex(PointerToSymbolTable));
		out.println(i + "  Symbol Count:         " + NumberOfSymbols);
		out.println(i + "  Optional Header Size: 0x"
				+ hex(SizeOfOptionalHeader));
