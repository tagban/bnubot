...
public class Ini {
	...
	public void save() {
		...
		try {
			...
			for (Enumeration keys = data.keys(); keys.hasMoreElements();) {
				...
				out.write((header + System.getProperty("line.separator"))
						.getBytes());
				// System.out.println(header);
				Properties section = data.get(header);
				for (Enumeration props = section.propertyNames(); props
						.hasMoreElements();) {
					String prop = props.nextElement().toString();
					String value = section.getProperty(prop);
					out.write((prop + "=" + value + System
	...
	public static String ReadIni(String FN, String Header, String Setting,
			String Default) {
			...
			while ((var = inputStream.readLine()) != null) {
				if (var.toLowerCase().equals("[" + Header.toLowerCase() + "]"))
					fndHdr = true;
						...
						&& (var.substring(0, 1).equals("[") == true))
					break;
				else if ((var.length() >= Setting.length() + 1)
						&& (fndHdr == true)
						&& var.toLowerCase().substring(0, Setting.length())
								.equals(Setting.toLowerCase()))
					return (var.length() == Setting.length() + 1) ? Default
							: var.substring(Setting.length() + 1, var.length());
			}
		} catch (FileNotFoundException e) {
		} catch (IOException ex) {
		}
	...
	public static void WriteIni(String FN, String Header, String Setting,
			String Value) {
