 ...
 * IpAuth Class
 * 
 * This Class Stores the IP Authorization/Banning list It Keeps Track of Who
 * Connects and when, and performs any necessary IPBanning
 * 
 * This Class and all members are static, in order to be accessible to all
 * connection threads
 * 
 */
public class IpAuth {
	...
	public static final int IPNORESTRICTIONS = 0;// All connections are
													// completly unrestricted
	public static final int IPBANNING = 1; // Enables IPBanning of IPs
	public static final int IPRESTRICTED = 2; // Only allows Authorized IPs to
												// connect
	public static final int IPLOCALONLY = 3; // Only allows Local IPs to
												// connect (prefix 10, 192, 127

	/***************************************************************************
	 * @param IP -
	 *            IP address to be checked, as a string *
	 * @return boolean whether the IP is allowed to connect*
	 * @SuppressWarnings("fallthrough") *
	 **************************************************************************/
	public static boolean checkAuth(String IP) {
		int thisIpStatus = Integer.parseInt(Ini.ReadIni("./ips.ini", IP,
				"Status", "-1"));
		if (thisIpStatus != -1) {
			int count = Integer.parseInt(Ini.ReadIni("./ips.ini", IP,
					"Connection Attempts", "0"));
			count++;
			Ini
					.WriteIni("./ips.ini", IP, "Last Connection", Out
							.getDatestamp());
	...
	public static String getList() {
		...
		for (String element : ips)
			s.append(getIpString(element)).append("\r");
		return s.toString();
	}
	...
	public static String getIpString(String IP) {
		...
		s.append(" Status: ");
		int status = Integer.parseInt(Ini.ReadIni("./ips.ini", IP, "Status",
				"0"));
