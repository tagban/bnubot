...
public class OutPacketBuffer extends Buffer {
	...
	/**
	 * Gets the Buffer in the form of a string Unused currently
	 */
	public String getStringBuffer() {// Unused?
		// int s=size();
		// return (char)(byte)((s+3 >> 0) & 0x000000FF) + (char)(byte)((s+3 >>
		// 8) & 0x000000FF) + String.valueOf(super.getBuffer());
		...
		for (byte element : ret) {// have to loop through to convert
									// everything to chars
	...
	/**
	 * Overrides the "Buffer" method, returning the packet ID/length and the
	 * data
	 * 
	...
	public byte[] getBuffer() {
		...
		System.arraycopy(super.getBuffer(), 0, ret, 3, size());// Copy Buffer
																// Into a New
																// Array
		...
		// System.out.println("Ret Size"+ret.length + "last
