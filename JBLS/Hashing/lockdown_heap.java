...
public class lockdown_heap {
	...
	public int[] toIntArray() {
		...
		for (int x = 0; x < (currentlength * 4); x++) {
			buff[x] = memory[x * 4] & 0x000000FF;
			buff[x] |= (memory[x * 4 + 1] << 8) & 0x0000FF00;
			buff[x] |= (memory[x * 4 + 2] << 16) & 0x00FF0000;
			buff[x] |= (memory[x * 4 + 3] << 24) & 0xFF000000;
		}
		return buff;
	}
	...
	public void resize() {
		...
		// System.out.println("Resized: " + Integer.toHexString(memory.length) +
		// " -> " + Integer.toHexString(currentlength*0x10));
	...
	public void add(char[] data) {
		if (currentlength + 0x10 >= maximumlength) {
			maximumlength *= 2;
			// System.out.println("Heap: " + Integer.toHexString(currentlength)
			// + " " + Integer.toHexString(maximumlength));
		...
		if (autosort) {
			...
			// System.out.print("Add " + insert + " " + getInt(data, 0) + " ");
			// utile.print_hash(data);
		} else {
	...
	private int findLocation(int num) {
		for (int x = -1; x < currentlength; x++) {
			// System.out.println(num + " " + getInt(x*0x10) + " " +
			// getInt((x+1)*0x10));
	...
	private int partition(int l, int r) {
		...
		while (i <= j) {
			...
			// Integer.toHexString(getInt(i*0x10)) + " " +
			// Integer.toHexString(getInt(j*0x10)));
			if (getInt(i * 0x10) <= p)
				i++;
			else if (getInt(j * 0x10) > p)
				j--;
			else
				swap(i, j);
		}
	...
	private void swap(int i, int j) {
		...
		// System.out.println( "Swap: " + Integer.toHexString(getInt(i*0x10)) +
		// " -> " + Integer.toHexString(getInt(j*0x10)));
	...
	public void print() {
		for (int i = 0; i < currentlength; i++) {
			for (int j = 0; j < 0x10; j++)
				System.out.print((memory[i * 0x10 + j] < 0x10 ? "0" : "")
						+ Integer.toHexString(memory[i * 0x10 + j]) + " ");
			System.out.print("\t");
			for (int j = 0; j < 0x10; j++)
				System.out.print(((memory[i * 0x10 + j] < 0x20)
						|| (memory[i * 0x10 + j] > 0x7F) ? "." : memory[i
						* 0x10 + j]));
			System.out.println("");
		}
	...
	private int getInt(int o) {
		return getInt(memory, o);
	}

	private int getInt(char[] m, int o) {
		if (o < 0)
			return 0;
		int sig = (m[o + 0] << 0) & 0x000000FF;
		sig |= (m[o + 1] << 8) & 0x0000FF00;
		sig |= (m[o + 2] << 16) & 0x00FF0000;
		sig |= (m[o + 3] << 24) & 0xFF000000;
		return sig;
	}
}