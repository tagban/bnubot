...
//    System.out.println(a + " " + mpq + " sdfgsdfgsdfgsdfgsdfgsdfgsdfg");
    ...
    for(int x = 0; x < 0x10; x++) returnbuff.add((byte)temp_memory[x]);
...
//    System.out.println("Hash File: " + filename);
    ...
    if(pe.ntheader.Signature != pe.IMAGE_NT_SIGNATURE) return; //Not a valid PE file
...
//    ld_heap.print();
    for (IMAGE_SECTION_HEADER element : pe.sectionHeaders)
		hash1(ctx, ld_heap, pe, PEImageBase, PEImageBaseUpper, element, PESectionAlignment, lockdown);
    //ld_heap.destroy();
  }
      ...
      if(dwBytes > 0){
        ...
        while(i < dwBytes){
          ...
          if(index < ld_heap.length()){
            ...
            eax = (var_40[0]-section.VirtualSize - section.VirtualAddress+allocated_memory_base[0]);
          }
          dwBytes += i;
          if(eax>0){
...
//        System.out.print("2: ");utile.print_hash(getCharArray(value));
  ...
  public static void process_reloc(lockdown_heap ld_heap, PEFile pe, boolean is32bit){
    ...
    int RelocSectionEnd = RelocSectionStart + pe.ntheader.OptionalHeader.DataDirectory[pe.RELOC_TABLE].Size;
