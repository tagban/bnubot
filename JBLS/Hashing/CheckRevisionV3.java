...
public class CheckRevisionV3 extends CheckRevisionV1 {
	...
	public static long word_shifter(int str1, int str2) {
		...
		str2 = (short) ((str2 & 0xFF00) | (((str2 + 1) & 0xFF) - (((str2 & 0xFF) != 0xFF) ? 1
				: 0)));

		str1 = (short) (((str1 - str2) & 0xFF) | (((((str1 - str2) >> 8) & 0xFF) + 1) > 0 ? 0
				: 0x100));
	...
	public static CheckrevisionResults checkRevision(String valuestring,
			int prod, byte plat, String mpq) throws FileNotFoundException,
			IOException {
		CheckrevisionResults cacheHit = crCache.get(valuestring + mpq + prod
				+ plat);
		if (cacheHit != null) {
			Out.println("CREV", "CheckRevision cache hit: " + crCacheHits
					+ " hits, " + crCacheMisses + " misses.");
			crCacheHits++;
			return cacheHit;
		}
		Out.println("CREV", "CheckRevision cache miss: " + crCacheHits
				+ " hits, " + crCacheMisses + " misses.");
		crCacheMisses++;

		// Buffer a = new Buffer(valuestring.getBytes());
		// System.out.println(a + " " + mpq + " sdfgsdfgsdfgsdfgsdfgsdfgsdfg");

		lockdown_SHA1 ctx = new lockdown_SHA1();
		char[] return_is_valid = new char[] { (char) 1, (char) 0, (char) 0,
				(char) 0 };
		char[] module_offset = new char[] { (char) 0, (char) 0, (char) 0,
				(char) 0 };
		...
		for (int x = 0; x < 0x40; x++) {
			valuestring_buffer_1[x] = '6';
			valuestring_buffer_2[x] = '\\';
		}
		...
		for (int x = 0; x < 0x10; x++)
			returnbuff.add((byte) temp_memory[x]);
		returnbuff.addByte((byte) 0);
		if (Version[plat][prod] == 0)
			Version[plat][prod] = getVersion(files, prod);

		CheckrevisionResults result = new CheckrevisionResults(
				Version[plat][prod], ld_sha1_out_buffer_2[0], returnbuff);
		crCache.put(valuestring + mpq.replaceAll("dll", "mpq") + prod + plat,
				result);
		return result;
	}

	public static void hash_file(lockdown_SHA1 ctx, String filename,
			String lockdown) throws FileNotFoundException, IOException {
		...
		if (pe.ntheader.Signature != pe.IMAGE_NT_SIGNATURE)
			return; // Not a valid PE file
		if (pe.ntheader.FileHeader.Characteristics < 0x00E0)
			return; // 0.o Wtf is flag 0x40?
		if (pe.ntheader.OptionalHeader.Magic == pe.IMAGE_NT_OPTIONAL_HDR32_MAGIC) {
			if (pe.ntheader.OptionalHeader.NumberOfRvaAndSizes <= 0x0D)
				return;
		...
		ecx = (ecx + pe.ntheader.OptionalHeader.FileAlignment - 1)
				& ~(pe.ntheader.OptionalHeader.FileAlignment - 1);
		ctx.update(pe.getData(0, ecx));

		if (ld_heap == null) {
			ld_heap = new lockdown_heap(true);
			if (pe.ntheader.OptionalHeader.DataDirectory[pe.RELOC_TABLE].VirtualAddress > 0)
		...
		// ld_heap.print();
		for (IMAGE_SECTION_HEADER element : pe.sectionHeaders)
			hash1(ctx, ld_heap, pe, PEImageBase, PEImageBaseUpper, element,
					PESectionAlignment, lockdown);
		// ld_heap.destroy();
	}

	public static void hash1(lockdown_SHA1 ctx, lockdown_heap ld_heap,
			PEFile pe, int preferred_baseaddr, int preferred_baseaddr_upper,
			IMAGE_SECTION_HEADER section, int section_alignment, String lockdown) {
		...
		dwBytes = ((section.VirtualSize + section_alignment - 1) & ~(section_alignment - 1))
				- section.VirtualSize;
		if (section.Characteristics < 0) {
			...
			index = 0;
			if (ld_heap.length() > 0)
				for (i = 0; (index < ld_heap.length())
						&& (lockdown_memory[i] < edi); i += 4)
					index++;
			if (section.VirtualSize > 0) {
				...
				while ((ptr_memory - starting_memory) < section.VirtualSize) {
					int section_length = starting_memory - ptr_memory
							+ section.VirtualSize;
					eax = 0;
					if (index < ld_heap.length())
						eax = (lockdown_memory[memory_offset] + starting_memory - section.VirtualAddress);
					...
					if (section_length > 0) {
						// System.out.println("Section_length_update: " +
						// PadString.padHex(ptr_memory, 8) + " " +
						// PadString.padHex(section_length, 8));
						// utile.print_hash(pe.getDataSection(ptr_memory,
						// section_length));
						ctx.update(pe
								.getDataSection(ptr_memory, section_length));
						ptr_memory += section_length;
					} else {
						int[] heap_buffer = new int[4];
						// System.out.println("Heap Size: " +
						// Integer.toHexString(lockdown_memory.length) + " " +
						// Integer.toHexString(memory_offset));
						// System.out.println(Integer.toHexString(lockdown_memory.length)
						// + " " + Integer.toHexString(memory_offset));
						System.arraycopy(lockdown_memory, memory_offset,
								heap_buffer, 0, 4);
						hash2(pe, preferred_baseaddr, preferred_baseaddr_upper,
								ctx, heap_buffer, ptr_memory, lockdown);
			...
			if (dwBytes > 0) {
				...
				while (i < dwBytes) {
					eax = 0;
					if (index < ld_heap.length()) {
						System.arraycopy(lockdown_memory, index * 4, var_40, 0,
								4);
						eax = (var_40[0] - section.VirtualSize
								- section.VirtualAddress + allocated_memory_base[0]);
					}
					dwBytes += i;
					if (eax > 0) {
					...
					if (dwBytes > 0) {
						...
						System.arraycopy(allocated_memory_base, i, mem, 0,
								dwBytes);
						ctx.update(mem);
						i += dwBytes;
					} else {
						System.arraycopy(lockdown_memory, index * 4, var_30, 0,
								4);
						hash2(pe, preferred_baseaddr, preferred_baseaddr_upper,
								ctx, var_30, allocated_memory_base[i], lockdown);
						index++;
						i += var_30[4];
					}
				}
	...
	private static void hash2(PEFile pe, int preferred_baseaddr,
			int preferred_baseaddr_upper, lockdown_SHA1 ctx,
			int[] offset_memory, int ptr_memory, String lockdown) {
		int cf = 1;
		int seedIndex = Integer.parseInt(lockdown.substring(
				lockdown.toLowerCase().indexOf("lockdown-ix86-")).substring(14,
				16));

		if ((seeds[seedIndex][0] == 0) || (seeds[seedIndex][1] == 0))
			seeds[seedIndex] = seed_finder.find_seeds(lockdown,
					seed_finder.lockdown_values, seed_finder.lockdown_offsets);

		if (offset_memory[2] == 0) {
			if (offset_memory[3] == 0) {
				ctx.pad(offset_memory[1]);
				// System.out.println("Padd: " +
				// Integer.toHexString(offset_memory[1]));
			} else {
				// System.out.print("0:
				// ");utile.print_hash(pe.getDataSection(offset_memory[3],
				// offset_memory[1]));
				ctx.update(pe
						.getDataSection(offset_memory[3], offset_memory[1]));
			}
			...
			if (offset_memory[3] == 3) { // 32-bit
				int value = 0;
				if (ptr_memory != 0)
					value = pe.getIntSection(ptr_memory) ^ seeds[seedIndex][0];
				ctx.update(getCharArray(value));
				// System.out.print("2:
				// ");utile.print_hash(getCharArray(value));
			}
		}
	}

	public static void process_reloc(lockdown_heap ld_heap, PEFile pe,
			boolean is32bit) {
		int RelocSectionStart = pe.ntheader.OptionalHeader.DataDirectory[pe.RELOC_TABLE].VirtualAddress;
		int RelocSectionEnd = RelocSectionStart
				+ pe.ntheader.OptionalHeader.DataDirectory[pe.RELOC_TABLE].Size;
		short edi;
		int edx, eax;

		if (RelocSectionEnd <= RelocSectionStart)
			return;
		// System.out.println("Start: " + Integer.toHexString(RelocSectionStart)
		// + " End: " + Integer.toHexString(RelocSectionEnd));
		...
		while (RelocSectionStart < RelocSectionEnd) {
			...
			while (var_8 > 0) {
				...
				if (eax != 0) {
					...
					DataArray[0] = pe.getIntSection(RelocSectionStart)
							+ (edi & 0x0FFF);
					DataArray[1] = edx;
					DataArray[2] = 2;
					DataArray[3] = eax;
	...
	public static void process_import(lockdown_heap ld_heap, PEFile pe,
			boolean is32bit) {
		int importaddress = pe.ntheader.OptionalHeader.DataDirectory[pe.IMPORT_TABLE].VirtualAddress;
		int importsize = pe.ntheader.OptionalHeader.DataDirectory[pe.IMPORT_TABLE].Size;
		int[] DataArray = new int[4];
		int var_8;
		int eax = 0, esi, ecx, edx;

		if ((importaddress <= 0) || (importsize < 0x14))
			return;
		var_8 = 8 - importaddress;
		esi = importaddress + 0x10;
		while (eax <= importsize) {
			if (pe.getIntSection(esi - 4) == 0)
				return;
	...
	public static String[] getFiles(int prod, byte plat) {
		...
		case Constants.PLATFORM_INTEL:
			...
			ret[3] = Constants.IX86files[prod - 1][0]
					+ Constants.IX86files[prod - 1][4]; // Screen dump
			break;
		default:
			ret = null;
		}
	...
	private static char[] getCharArray(int array) {
		return getCharArray(new int[] { array });
	}
	...
	private static char[] getCharArray(int[] array) {
		...
		for (int element : array) {
			newArray[pos++] = (char) ((element >> 0) & 0xFF);
			newArray[pos++] = (char) ((element >> 8) & 0xFF);
			newArray[pos++] = (char) ((element >> 16) & 0xFF);
			newArray[pos++] = (char) ((element >> 24) & 0xFF);
		}
		return newArray;
	}
}