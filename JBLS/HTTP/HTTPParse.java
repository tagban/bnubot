      ...
      if(reqType.equalsIgnoreCase("GET")){
		  ...
		  while (bCurrent != null) {
		    ...
		    bCurrent = bCurrent.getNext();
       	  ...
       	  page = page.replaceAll("<admin>", ""+AdminConnectionThread.connectionCount);
...
/*       	  page = page.replaceAll("<starvb>", hex(Constants.verbyteSTAR));
  ...
  private void sendHashFile(String fileName, String[] files){
	...
	try{
      FileInputStream fis = new  FileInputStream("./zips/" + fileName+".txt");
      int size = fis.available();
      byte[] buff = new byte[size];
      fis.read(buff);
    ...
    HTTPSendFile sf = new HTTPSendFile(out, "./zips/" + fileName + ".zip", threadID);
    sf.run();
  }
